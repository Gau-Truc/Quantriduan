...
<html>
    ...
    <body>
        ...
        if (!empty($_SESSION['current_user'])) {
            ...
            $payments = mysqli_query($con, "SELECT orders.name, orders.address, orders.phone, orders.note, order_detail.*, product.name as product_name 
FROM orders
...
WHERE orders.id = " . $_GET['id']);
        ...
        <div id="order-detail-wrapper">
            <div id="order-detail">
                ...
                <label>Người nhận: </label><span> <?= $payments[0]['name'] ?></span><br/>
                <label>Điện thoại: </label><span> <?= $payments[0]['address'] ?></span><br/>
                <label>Địa chỉ: </label><span> <?= $payments[0]['phone'] ?></span><br/>
                <hr/>
                ...
                <p><label>Ghi chú: </label><?= $payments[0]['note'] ?></p>
            </div>
        </div>
    </body>
</html>